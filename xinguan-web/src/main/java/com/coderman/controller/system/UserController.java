...
public class UserController {
    ...
    @ApiOperation(value = "用户信息", notes = "用户登入信息")
    @GetMapping("/me")
    public ResponseBean<UserVO> me() throws SystemException {
        UserVO user = userService.findMe();
     ...
     * @return
     */
    @ApiOperation(value = "编辑个人信息", notes = "获取用户的详情，编辑用户信息")
    @GetMapping("/me/edit")
    @RequiresPermissions({"me:edit"})
    public ResponseBean<UserEditVO> meEdit() throws SystemException {
        ActiveUser activeUser = (ActiveUser) SecurityUtils.getSubject().getPrincipal();
        UserEditVO userVO = userService.edit(activeUser.getUser().getId());
     ...
     * @return
     */
    @ControllerEndpoint(exceptionMessage = "更新用户失败", operation = "更新个人信息")
    @ApiOperation(value = "更新用户", notes = "更新用户信息")
    @RequiresPermissions({"me:update"})
    @PutMapping("/me/update")
    public ResponseBean meUpdate( @RequestBody @Validated UserEditVO userEditVO) throws SystemException {
        ActiveUser activeUser = (ActiveUser) SecurityUtils.getSubject().getPrincipal();
        userService.update(activeUser.getUser().getId(), userEditVO);
     ...
     * @return
     */
    @ControllerEndpoint(exceptionMessage = "更新用户密码失败", operation = "更新用户密码")
    @ApiOperation(value = "更新用户密码", notes = "更新用户密码")
    @PutMapping("/pwd/update")
    public ResponseBean pwdUpdate( @RequestBody @Validated UpdatePwdVO updatePwdVO) throws BusinessException {
        ActiveUser activeUser = (ActiveUser) SecurityUtils.getSubject().getPrincipal();
        userService.updatePwd(activeUser.getUser().getId(), updatePwdVO);
        return ResponseBean.success();
    }
     ...
     * @return
     */
    @ControllerEndpoint(exceptionMessage = "更新用户黑名单状态失败", operation = "用户黑名单 ")
    @ApiOperation(value = "用户状态", notes = "禁用和启用这两种状态")
    @RequiresPermissions({"user:blacklist"})
    @PutMapping("/updateBlackListStatus/{id}/{status}")
    public ResponseBean updateBlackListStatus(@PathVariable Long id, @PathVariable Boolean status) throws SystemException {
        userService.updateBlackListStatus(id, status);
        return ResponseBean.success();
    }
