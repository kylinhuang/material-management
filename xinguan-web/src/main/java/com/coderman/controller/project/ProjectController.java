...
public class ProjectController {
    ...
    public ResponseBean findProjectList(@RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
                                        @RequestParam(value = "pageSize") Integer pageSize,
                                        @RequestParam(value = "categorys", required = false) String categorys,
                                        ProjectVO projectVO) {
        buildCategorySearch(categorys, projectVO);
        PageVO<ProjectVO> productVOPageVO = projectService.findProjectList(pageNum, pageSize, projectVO);
    ...
    public ResponseBean findMyProjectList(
...
//        }
    ...
    public ResponseBean add(@RequestBody @Validated ProjectVO projectVO) throws BusinessException {
        ...
        ProjectVO  res = projectService.add(projectVO);
...
//        return ResponseBean.success();
    ...
    @RequiresPermissions({"project:info"})
    @GetMapping("/info/{id}")
    public ResponseBean info(@PathVariable Long id) {
        ProjectVO productVO = projectService.info(id);
    ...
    public ResponseBean update(@PathVariable Long id, @RequestBody ProjectVO projectVO) throws BusinessException {
//        if (projectVO.getCategoryKeys().length != 3) {
//            throw new BusinessException(BusinessCodeEnum.PARAMETER_ERROR,"物资需要3级分类");
//        }
        projectService.update(id, projectVO);
        return ResponseBean.success();
    }
