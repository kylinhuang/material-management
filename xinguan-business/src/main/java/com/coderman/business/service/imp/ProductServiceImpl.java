...
public class ProductServiceImpl implements ProductService {
    ...
    public PageVO<ProductVO> findProductList(Integer pageNum, Integer pageSize, ProductVO productVO) {
        ...
        if (productVO.getStatus() == null || -1 == productVO.getStatus().intValue() ) {
    ...
    public void add(ProductVO ProductVO) {
        ...
        if(categoryKeys.length>=2) product.setTwoCategoryId(categoryKeys[1]);
        if(categoryKeys.length>=3) product.setThreeCategoryId(categoryKeys[2]);

//        product.setStatus(2);//未审核

        ActiveUser activeUser = (ActiveUser) SecurityUtils.getSubject().getPrincipal();
        if (activeUser.getUser().getRoleId()== RoleEnum.supplier.getId()){
            product.setStatus(ProductStatusEnum.create.getId());
        }else {
            product.setStatus(ProductStatusEnum.reviewed.getId());
        }

        product.setUserId(activeUser.getUser().getId());
    ...
    public void update(Long id, ProductVO productVO) throws BusinessException {
        ...
        if (activeUser.getUser().getRoleId()== RoleEnum.supplier.getId()){ // 供应商
            ...
            if (activeUser.getUser().getId().longValue() != productVO.getUserId().longValue()){

                throw new BusinessException(BusinessCodeEnum.PRODUCT_OPERATING_ERROR);
    ...
    public void remove(Long id) throws BusinessException {
        ...
        ActiveUser activeUser = (ActiveUser) SecurityUtils.getSubject().getPrincipal();
        if (activeUser.getUser().getRoleId()== RoleEnum.supplier.getId()){ // 供应商
            if (activeUser.getUser().getId().longValue() != product.getUserId().longValue()){
                throw new BusinessException(BusinessCodeEnum.PRODUCT_OPERATING_ERROR);
            }
        }


        if(ProductStatusEnum.delete.getId() == product.getStatus().intValue()){
            throw new BusinessException(BusinessCodeEnum.PRODUCT_STATUS_ERROR);
        }else {
            t.setStatus(ProductStatusEnum.delete.getId());
    ...
    public void back(Long id) throws BusinessException {
        ...
        // 0:逻辑删除,1:创建,2:审核中 ,3:审核通过
        if(ProductStatusEnum.delete.getId() != product.getStatus().intValue()){
            throw new BusinessException(BusinessCodeEnum.PRODUCT_STATUS_ERROR);
        }else {
            t.setStatus(ProductStatusEnum.create.getId());
            productMapper.updateByPrimaryKeySelective(t);
        }
    }
    ...
    public void publish(Long id) throws BusinessException {
        ...
        t.setId(id);
        Product product = productMapper.selectByPrimaryKey(t);
        if(ProductStatusEnum.reviewed.getId() == product.getStatus().intValue() ){
            throw new BusinessException(BusinessCodeEnum.PRODUCT_STATUS_ERROR);
        }else {
            t.setStatus(ProductStatusEnum.reviewed.getId());
            productMapper.updateByPrimaryKeySelective(t);
        }
    }
     ...
     */
    @Override
    public void submitReview(Long id) throws BusinessException {
        Product t = new Product();
        t.setId(id);

//        Product product = productMapper.selectByPrimaryKey(t);
        t.setStatus(ProductStatusEnum.reviewing.getId());
        productMapper.updateByPrimaryKeySelective(t);
