...
public class UserServiceImpl implements UserService {
    ...
    public UserVO findMe() {
        ActiveUser activeUser = (ActiveUser) SecurityUtils.getSubject().getPrincipal();
        User user = userMapper.selectByPrimaryKey(activeUser.getUser().getId());
        UserVO userVO = userConverter.converterToUserVO(user);
        return userVO;
    }
    ...
    @Override
    public void updateBlackListStatus(Long id, Boolean status) throws SystemException {
        User dbUser = userMapper.selectByPrimaryKey(id);
        if(dbUser==null){
            throw new SystemException(SystemCodeEnum.PARAMETER_ERROR,"要更新状态的用户不存在");
        }
    ...
    public void add(UserVO userVO) throws SystemException {
        ...
        @NotNull(message = "角色id不能为空") Long roleId = userVO.getRoleId();
        Example o = new Example(User.class);
        ...
        if (null != userList && userList.size() > 0){
            Long[] ids = {userVO.getRoleId()};
            assignRoles(userList.get(0).getId(),ids);
        }
    ...
    public void update(Long id, UserEditVO userVO) throws SystemException {
        User dbUser = userMapper.selectByPrimaryKey(id);
        @NotBlank(message = "用户名不能为空") String username = userVO.getUsername();
        @NotNull(message = "角色不能为空") Long roleId = userVO.getRoleId();
        if(dbUser==null){
            throw new SystemException(SystemCodeEnum.PARAMETER_ERROR,"要删除的用户不存在");
        }
        Role role = roleMapper.selectByPrimaryKey(roleId);
    ...
    @Override
    public void updatePwd(Long id, UpdatePwdVO updatePwdVO) throws BusinessException {
        User dbUser = userMapper.selectByPrimaryKey(id);

        String salt = dbUser.getSalt();

        String  pwd = MD5Utils.md5Encryption(updatePwdVO.getOldPassword(), salt);
        if (dbUser.getPassword().equalsIgnoreCase(pwd)){
            dbUser.setPassword(MD5Utils.md5Encryption(updatePwdVO.getNewPassword(), salt));
            userMapper.updateByPrimaryKeySelective(dbUser);

        }else {
            throw  new BusinessException(BusinessCodeEnum.PRODUCT_PWD_ERROR);
    ...
    public UserInfoVO info() throws SystemException {
        ...
        userInfoVO.setUserId(activeUser.getUser().getId());
        userInfoVO.setRoleId( activeUser.getUser().getRoleId());
        return userInfoVO;
    }
}
